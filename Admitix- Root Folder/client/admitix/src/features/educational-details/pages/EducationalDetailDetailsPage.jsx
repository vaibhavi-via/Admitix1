...
import { getEducationalDetailsById, deleteEducationalDetail } from '../services'
import { educationalDetailsFields } from '../constants'

export default function EducationalDetailDetailsPage() {
  const { id } = useParams()
  ...
  return (
    <div>
      <PageHeader
        title="Educational Detail Details"
        ...
        actions={
          <>
            ...
            <button
              onClick={() => navigate(`/educational-details/${id}/edit`)}
              className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
            >
              <Pencil size={16} />
              Edit
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5">
        ...
        {!isLoading && record && (
          <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {educationalDetailsFields.map((f) => (
