...
import { getAdmissionCyclesById, deleteAdmissionCycle } from '../services'
import { admissionCyclesFields } from '../constants'

export default function AdmissionCycleDetailsPage() {
  const { id } = useParams()
    ...
    <div>
      <PageHeader
        ...
        actions={
          <>
            <button
              onClick={() => navigate('/admission-cycles')}
            ...
            <button
              onClick={() => navigate(`/admission-cycles/${id}/edit`)}
              className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
            >
              <Pencil size={16} />
              Edit
