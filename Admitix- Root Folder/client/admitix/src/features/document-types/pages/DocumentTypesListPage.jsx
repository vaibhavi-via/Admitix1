...
import { getDocumentTypesList, deleteDocumentType } from '../services'
...
export default function DocumentTypesListPage() {
  ...
  const load = () => {
    ...
    getDocumentTypesList()
      ...
      .catch((err) => setError(err.message || 'Failed to load document types.'))
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        isLoading={isLoading}
        error={error}
        onView={(row) => navigate(`/document-types/${row.id}`)}
        onEdit={(row) => navigate(`/document-types/${row.id}/edit`)}
        onDelete={(row) => setPendingDelete(row)}
        emptyMessage="No document types found yet."
      />
