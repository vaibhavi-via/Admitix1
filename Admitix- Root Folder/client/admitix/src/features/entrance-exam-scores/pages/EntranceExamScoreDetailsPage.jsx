...
import { getEntranceExamScoresById, deleteEntranceExamScore } from '../services'
import { entranceExamScoresFields } from '../constants'

export default function EntranceExamScoreDetailsPage() {
  const { id } = useParams()
  ...
  return (
    <div>
      <PageHeader
        title="Entrance Exam Score Details"
        subtitle="Full record detail."
        actions={
          <>
            <button
              onClick={() => navigate('/entrance-exam-scores')}
            ...
            <button
              onClick={() => navigate(`/entrance-exam-scores/${id}/edit`)}
              className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
            >
              <Pencil size={16} />
              Edit
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5">
        ...
        {!isLoading && record && (
          <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {entranceExamScoresFields.map((f) => (
