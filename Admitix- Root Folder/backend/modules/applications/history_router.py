...
async def list_status_history(db: Session = Depends(get_db)):
    return get_application_status_history(db)
...
async def get_status_history(history_id: UUID, db: Session = Depends(get_db)):
    return get_application_status_history_by_id(db, history_id)
